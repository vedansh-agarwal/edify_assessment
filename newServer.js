...
app.post("/quizapi/customer/enter-email", (req, res) => {
  ...
  db.query(
    "SELECT registrationStatus FROM customers WHERE companyEmailId = ?",
    [email],
    async (err, result) => {
      if (err) {
        console.log(err);
        ...
        if (result.length > 0) {
          ...
          var hashedEmail =
            mailParts[0].substring(0, 2) +
            new Array(mailParts[0].length - 3).join("*") +
            mailParts[0].substring(mailParts[0].length - 2) +
            "@" +
            mailParts[1];
        ...
        var error;
        db.query(
          "INSERT INTO customer_otp (email, otp) VALUES (?, ?)",
          [email, otp],
          (err1) => {
            if (err1) {
              console.log(err);
...
app.post("/quizapi/customer/verify-otp", (req, res) => {
  ...
  db.query(
    "SELECT * FROM customer_otp WHERE email = ? AND otp = ?",
    [email, otp],
    (err, result) => {
      if (err) {
        console.log(err);
        ...
        var refreshToken = jwt.sign({ email }, process.env.JWT_REFRESH_SECRET, {
          expiresIn: "30d",
        });
        var accessToken = jwt.sign({ email }, process.env.JWT_ACCESS_SECRET, {
          expiresIn: "15m",
        });
        db.query(
...
app.post("/quizapi/customer/get-details", checkAuth, (req, res) => {
  ...
  db.query(
    "SELECT * FROM customers WHERE companyEmailId = ?",
    [email],
    (err, result) => {
      if (err) {
        console.log(err);
        ...
        if (result.length > 0) {
          return res
            .status(200)
            .json({ message: "User details", data: result[0] });
        } else {
...
app.post("/quizapi/customer/update-details", checkAuth, (req, res) => {
  ...
  companyUrl = companyUrl || null;
  db.query(
    "UPDATE customers SET companyName = ?, customerName = ?, mobileNo = ?, designation = ?, country = ?, companyUrl = ?, updatedOn = CURRENT_TIMESTAMP() WHERE companyEmailId = ?",
    [
      companyName,
      customerName,
      mobileNo,
...
app.post("/quizapi/customer/logout", checkAuth, (req, res) => {
  ...
  if (!isComplete) {
    db.query(
      "UPDATE customers SET refreshToken = null WHERE companyEmailId = ?",
      [email],
      (err) => {
        if (err) {
          check = err;
  ...
  if (check)
    return res
      .status(299)
      .json({ message: "Database Error", errMsg: check.message });

  db.query(
    "SELECT customerId from customers WHERE companyEmailId = ?",
    [email],
    async (err, resp) => {
      if (err) {
        console.log(err);
        ...
        db.query(
          "INSERT INTO survey_answer (customerId, surveyAnswers, surveyEndDate, surveyCompleteFlag) VALUE (?, ?, " +
          ...
          (err1, result) => {
            if (err1) {
              ...
              return res
                .status(299)
                .json({ message: "Database Error", errMsg: err1.message });
            } else {
              return res
                .status(200)
                .json({ message: "Survey answers saved successfully" });
            }
          }
        );
      } else {
